...
for (int i = 0; i < 4; i++)
for (int j = 0; j < 4; j++)
image(backgroundImg, width * (i - 1.5) + (-Player_id.XY.x % width),
...
if (weapon_mode % 2 == 1) { fill(100, 0, 0); text("已解鎖", width/2, height/2 + 175); fill(0); }

text("每過一段時間隨機鎖定一個敵人射擊", width/2, height/2 + 100);
if (weapon_mode % 4 > 1) { fill(100, 0, 0); text("已解鎖", width/2, height/2 + 75); fill(0); }

text("按下空白鍵，朝四面八方散射書本", width/2, height/2);
if (weapon_mode % 8 > 3) { fill(100, 0, 0); text("已解鎖", width/2, height/2 - 25); fill(0); }

text("射出去的書本不會立即消失，將會停留在原地造成持續傷害，持續一段時間",
width/2, height/2 - 100);
if (weapon_mode % 16 > 7) { fill(100, 0, 0); text("已解鎖", width/2, height/2 - 125); fill(0); }

text("書本觸碰到敵人後將不會消失，改為貫穿傷害", width/2, height/2 - 200);
if (weapon_mode % 32 > 15) { fill(100, 0, 0); text("已解鎖", width/2, height/2 - 225); fill(0); }
